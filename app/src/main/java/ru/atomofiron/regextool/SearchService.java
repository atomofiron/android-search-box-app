...
public class SearchService extends IntentService {
    ...
    ArrayList<String> arrayOfLineCounts = new ArrayList<>();
    Pattern pattern;

    boolean done = false; // у меня на Meizu без этого сервис при его закрытии не умирает
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (!caseSense) target = target.toLowerCase();
        pattern =caseSense ?
        ...
        simple = intent.getBooleanExtra(I.SEARCH_SIMPLE,false);

        startForeground();
        maxSize=getBaseContext().getSharedPreferences(I.PREFS,MODE_PRIVATE).getInt("max_size",maxSize);
        try {
            for (File file : Strings2Files(intent.getStringArrayListExtra(I.SEARCH_LIST)))
                if (inFiles) searchInFiles(file); else search(file);
            if (!done) sendResults(I.SEARCH_OK);
        } catch (Exception e) {
            I.Log(e.getMessage());
            if (!done) sendResults(I.SEARCH_ERROR);
        }
        done = true;
    }
    File[] Strings2Files(ArrayList<String> stringsList) {
        ...
        for (int i=0;i<n;i++) filesList[i]=new File(stringsList.get(i));
        return filesList;
    }

    void search(File file) {
        if (pattern.matcher(file.getName()).find()) resultsList.add(file.getAbsolutePath());
        if (file.isDirectory()) {
            File[] files = file.listFiles();
            if (file.listFiles()!=null && file.listFiles().length>0)
                for (File nextfile : files) search(nextfile);
        }
    }
    void searchInFiles(File file) {
        if (file.isDirectory()) {
            File[] files = file.listFiles();
            if (file.listFiles()!=null && file.listFiles().length>0)
                for (File nextfile : files) searchInFiles(nextfile);
                ...
                case "json":
                    ...
                    try {
                        ...
                        while (!end || (line = br.readLine()) != null) { // хитрая конструкция
                            ...
                            if (!caseSense) line = line.toLowerCase();
                            if (simple) {
                        ...
                        if (k>0) {
                            ...
                            resultsListLines.add("("+k+") ");
                            arrayOfLineCounts.add(lineCounts);
                        }
    ...
    void sendResults(int code) {
        ...
        if (code == I.SEARCH_OK)
            intent
                    ...
                    .putExtra(I.RESULT_LIST_LINES, resultsListLines)
                    .putExtra(I.RESULT_LIST_LINE_COUNTS, arrayOfLineCounts);
    ...
    void startForeground() {
        ...
        Notification notif;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) notif = builder.build();
        else notif = builder.getNotification();
