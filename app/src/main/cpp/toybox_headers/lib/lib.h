...
void llist_traverse(void *list, void (*using_)(void *node));
...
void dlist_add_nomalloc(struct double_list **list, struct double_list *new_);
...
int dirtree_notdotdot(struct dirtree *catch_);
...
long long nanodiff(struct timespec *old, struct timespec *new_);
...
void comma_collate(char **old, char *new_);
